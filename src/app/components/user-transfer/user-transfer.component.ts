...
import { accountList } from 'src/app/models/accountList';
...
export class UserTransferComponent implements OnInit {
  ...
  OnSubmit(){
    if(this.accIdSource!=null && this.accIdSource>0 && this.accIdDest!=null && this.accIdDest>0 && this.amount!=null && this.amount>0)
    {
      console.log("Form is submitted"+this.accIdDest)
      this.transactionService.transferUser(this.accIdSource,this.accIdDest,this.amount).subscribe(
        (response:any)=>{
